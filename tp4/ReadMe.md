## TP4 - Administration Base de Données - Daoud Anas - 2CS4 
...
```
  --->  **Dès que l'utilisateur est créé, le DBA peut accorder les privilèges de système spécifiques à cet utilisateur lui meme**
 

  - **Donner les privilèges ci-dessous à l'utilisateur dev1 :** 
...
```
  ...
  - **Créer des rôles pour chaque Equipe d'utilisateurs en réspectant les critères ci-dessous** 

      A) Le rôle de l'équipe Dev peut faire:
      ...
      * Création,lecture, modification de structure et suppression de tables.
      ...
      B) Le rôle de l'équipe Test peut faire:

      * Se connecter à la base de données.
      * Création de session.
      * Lecture données de toutes les tables.
     
     C) Le rôle de l'équipe DevSecOps nous permet d'avoir tous les privilèges avec mode administrateur de la base:  
...
```
   ...
   - **Accorder à chaque utilisateur, le rôle qui lui correspond:** 
...
```
   ...
   - **Donner l'autorisation tous les utilisateurs sur le système pour interroger les données de la table EMP :** 
...
**Création d'un profile de ressources pour l'équipe des développeurs avec les limitations suivantes:**
